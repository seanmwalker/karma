    ...
    buckets.forEach(function(bucket, idx) {
      bucket.sort(byPath).forEach(function(file) {
        if (!uniqueMap[file.path]) {
          ...
          if (patterns[idx].included && patterns[idx].insertBeforeFramework) {
            files.includedBeforeFramework.push(file);
          }
          else if (patterns[idx].included) {
