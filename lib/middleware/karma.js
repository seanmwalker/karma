...
var buildScriptTags = function(fileList) {
    ...
    if (requestUrl === '/context.html' || requestUrl === '/debug.html') {
      return filesPromise.then(function(files) {
        serveStaticFile(requestUrl, response, function(data) {
          ...
          var libScriptTags = buildScriptTags(files.includedBeforeFramework);
          var scriptTags = buildScriptTags(files.included);
          ...
          return data.replace('%PRELIBSCRIPTS%', libScriptTags).replace('%SCRIPTS%', scriptTags.join('\n')).replace('%MAPPINGS%', mappings);
        });
      });
    }
